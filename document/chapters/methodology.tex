...
Equations \ref{eq:kalmangain}, \ref{eq:new_state_measurement}, and \ref{eq:new_uncertainty} are used. At the start, a value needs to be assumed for the underlying uncertainty in the GEBCO data. This value was set empirically to 1.5m. 
...
This provides a dataset of the ocean clarity (as estimated by Secchi Depth) (per GlobColour) data at the site \emph{on the day of the satellite pass.} Statistics can then be calculated for each unique trackline, or by each site as a whole.
